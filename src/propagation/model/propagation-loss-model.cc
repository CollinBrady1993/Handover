...
// ------------------------------------------------------------------------- //
// -- Overwrite Loss Model -- collinb@uw.edu ------------------------------- //

NS_OBJECT_ENSURE_REGISTERED (ManualOverwritePropagationLossModel);

TypeId 
ManualOverwritePropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ManualOverwritePropagationLossModel")
    .SetParent<PropagationLossModel> ()
    .SetGroupName ("Propagation")
    .AddConstructor<ManualOverwritePropagationLossModel> ()
    .AddAttribute ("Frequency", 
                   "The carrier frequency (in Hz) at which propagation occurs  (default is 5.15 GHz).",
                   DoubleValue (5.150e9),
                   MakeDoubleAccessor (&ManualOverwritePropagationLossModel::SetFrequency,
                                       &ManualOverwritePropagationLossModel::GetFrequency),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("SystemLoss", "The system loss",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&ManualOverwritePropagationLossModel::m_systemLoss),
                   MakeDoubleChecker<double> ())
  ;
  return tid;
}

ManualOverwritePropagationLossModel::ManualOverwritePropagationLossModel ()
{
}
void
ManualOverwritePropagationLossModel::SetSystemLoss (double systemLoss)
{
  m_systemLoss = systemLoss;
}
double
ManualOverwritePropagationLossModel::GetSystemLoss (void) const
{
  return m_systemLoss;
}
void
ManualOverwritePropagationLossModel::SetMinDistance (double minDistance)
{
  m_minDistance = minDistance;
}
double
ManualOverwritePropagationLossModel::GetMinDistance (void) const
{
  return m_minDistance;
}
void
ManualOverwritePropagationLossModel::SetHeightAboveZ (double heightAboveZ)
{
  m_heightAboveZ = heightAboveZ;
}

void
ManualOverwritePropagationLossModel::SetFrequency (double frequency)
{
  m_frequency = frequency;
  static const double C = 299792458.0; // speed of light in vacuum
  m_lambda = C / frequency;
...
double
ManualOverwritePropagationLossModel::GetFrequency (void) const
{
  return m_frequency;
}

double 
ManualOverwritePropagationLossModel::DbmToW (double dbm) const
{
  double mw = std::pow (10.0,dbm / 10.0);
  return mw / 1000.0;
}
...
double
ManualOverwritePropagationLossModel::DbmFromW (double w) const
{
  double dbm = std::log10 (w * 1000.0) * 10.0;
  return dbm;
}
...
double 
ManualOverwritePropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                                    Ptr<MobilityModel> a,
                                                    Ptr<MobilityModel> b) const
{
  /*
   * This pathloss model overwrites the transmission power with a power spectral density value calculated outside of NS-3.
   * This model allows for each link between an eNb and UE to have a separate channel response.
   * The input files must be formatted as an ascii file with numRows = numRB and one data point each millisecond
   */
  
//should need some type of ID tag for the eNb and the UE and thats it

  //*
  int RB = 15;// this is the targer RB and will be based on an input to the function, this may need to be replaced if the function is called on all RB simultaniously
  int timeStamp = 10;// this is the index of the target timestamp and will be based on an input to the function, calculated by currentSimTime/.001

  std::ifstream PHYTrace("TX1_Sector1.txt");// needs to be replaced with a filename based upon the TX/UE combo, specific filepath tbd.
  std::string line;
  int rbIndex = 0;
  double val;
  double save = 0;
  ...
  while(std::getline(PHYTrace, line))
  {
  ...
  return save;// ultimately this will return the value "save"
}

int64_t
ManualOverwritePropagationLossModel::DoAssignStreams (int64_t stream)
{
  return 0;
}
